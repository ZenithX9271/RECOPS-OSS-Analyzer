...
# -*- coding: utf-8 -*-
"""
RECOPS Scorecard Extractor
==========================

Scores power-system open-source repositories on the six RECOPS families /
27 sub-indicators, and lets you interactively ask an LLM about the results.

For every repository it:
  1. Clones the repo ONCE (full history + tags) and reuses that single clone
     across all six families.
  2. Runs the six RECOPS families:
        EQ  - Engineering Quality        (CQI, ARS, PER, SPS, SRSW)
        AE  - Adoption & Ecosystem        (CEI, EVS, RWIS, AQS)
        GC  - Governance & Credibility    (GOS, SCS, ITI, RVS)
        DI  - Deployability & Interop.     (DRS, ICI, PFS, OBS)
        PH  - Project Health               (PVI, MQS, FCS, MRD, BUS_FACTOR)
        EDF - Energy-Domain Fitness        (ESCI, CPRI, ENSC, EDVS)
  3. Collects GitHub + local metadata.
  4. Optionally writes a per-repo plain-English summary from the computed
     numbers, and exposes an interactive chat panel over all results.
  5. Exports CSV / Excel / JSON.

USAGE
-----
GUI:   streamlit run app.py
CLI:   python app.py https://github.com/owner/repo [more urls]

IMPORTANT: the six recops_repo_scores_*.py modules must live in the SAME
folder as this file (or one folder below it). If they are missing, the app
shows a clear banner telling you where it looked.

CREDENTIALS
-----------
No credentials are hard-coded in this file. The GitHub token and Groq key
are resolved at runtime from, in order: an environment variable, then
Streamlit secrets (st.secrets). If neither is set, the sidebar shows an
empty password field so a user can paste a key for that session only.
...
# =========================================================================== #
#               CREDENTIAL RESOLUTION -- env var, then st.secrets             #
# =========================================================================== #
...
# Default Groq models offered in the UI (first is the default).
...
# Per-family wall-clock limits inside run_all_families (seconds).
DEFAULT_FAMILY_TIMEOUT_S = 900
PH_FAMILY_TIMEOUT_S = 1800
_MODULE_CACHE: Dict[str, Any] = {}


def _family_timeout_s(family: str, default_timeout_s: int = DEFAULT_FAMILY_TIMEOUT_S) -> int:
    """PH runs five GitHub-heavy sub-scorers; allow it more time than other families."""
    return PH_FAMILY_TIMEOUT_S if family == "PH" else default_timeout_s


def _find_module_file(stem: str) -> Optional[Path]:
    """Search the app dir, cwd, and one level of subdirectories for <stem>.py."""
...
def _get_module(family: str):
    """Import (and cache) a scoring module by family code, with a clear error."""
    ...
    try:
        ...
        if not path:
            ...
            raise ModuleNotFoundError(
                ...
                f"files in the same folder as app.py. Looked in: {looked}"
            )
...
def verify_modules() -> Dict[str, Optional[str]]:
    """Return {family: None if importable else error string}. Used by the UI."""
...
# --------------------------------------------------------------------------- #
# Run a single family / all families against an already-cloned repo          #
# --------------------------------------------------------------------------- #
...
def run_all_families(repo_path: Path, repo_url: str, token: Optional[str],
                     ...
                     per_family_timeout_s: int = DEFAULT_FAMILY_TIMEOUT_S) -> Dict[str, Dict[str, Any]]:
    families = families or ALL_FAMILIES
    results: Dict[str, Dict[str, Any]] = {}
    for fam in families:
        timeout_s = _family_timeout_s(fam, per_family_timeout_s)
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
            fut = ex.submit(run_family, fam, repo_path, repo_url, token, aggregation)
            try:
                results[fam] = fut.result(timeout=timeout_s)
            except concurrent.futures.TimeoutError:
                results[fam] = {"error": f"timeout after {timeout_s}s"}
            except Exception as e:
...
# --------------------------------------------------------------------------- #
# Flatten nested family results into one wide row                            #
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# Compact context builder for the LLM (keeps token use small)                #
# --------------------------------------------------------------------------- #
def build_llm_context(flat_rows: List[Dict[str, Any]], max_repos: int = 25) -> str:
    """A small JSON block of scores+key metadata for the LLM to reason over."""
...
def llm_narrative(repo_url: str, flat_row: Dict[str, Any],
    ...
    try:
        ...
        prompt = ChatPromptTemplate.from_template(
            ...
            "and were computed deterministically from the repository, git history and "
            "GitHub API. METADATA: {meta}\n\nFAMILY SCORES:\n{signals}\n\n"
...
def llm_chat_answer(question: str, context_json: str,
    ...
    """Answer a free-form question about the scorecard, with prior turns as context."""
    try:
        from langchain_groq import ChatGroq
...
# --------------------------------------------------------------------------- #
# End-to-end per-repository processing                                        #
# --------------------------------------------------------------------------- #
def process_single_repo(repo_url: str,
                        ...
                        per_family_timeout_s: int = DEFAULT_FAMILY_TIMEOUT_S) -> Dict[str, Any]:
...
def _run_cli(argv: List[str]) -> int:
    ...
    ap = argparse.ArgumentParser(
        description="RECOPS Scorecard Extractor (headless). Clones each repo once "
                    "and runs all six RECOPS families.")
    ap.add_argument("urls", nargs="+", help="One or more GitHub repository URLs")
    ap.add_argument("--token", default=resolve_github_token())
    ap.add_argument("--groq-key", default=resolve_groq_key())
    ap.add_argument("--families", default="all",
                    help="Comma-separated subset of EQ,AE,GC,DI,PH,EDF or 'all'")
    ...
    # Fail fast with a clear message if the modules are missing.
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, args.workers)) as ex:
        ...
        for fut in concurrent.futures.as_completed(futures):
            u = futures[fut]
            res = fut.result()
            flat_rows.append(res["flat"])
            full_blobs.append(res["full"])
    ...
    try:
        ...
        if args.out:
            ...
            print(f"[RECOPS] CSV written to {args.out}")
        else:
            print(df.to_string(index=False))
    except Exception as e:
        print(f"[RECOPS] (pandas unavailable: {e}); dumping JSON")
        print(json.dumps(flat_rows, indent=2, default=str))

    if args.json_out:
        Path(args.json_out).write_text(json.dumps(full_blobs, indent=2, default=str),
                                       encoding="utf-8")
        print(f"[RECOPS] Full JSON written to {args.json_out}")
    return 0
...
#                            STREAMLIT  GUI  APP                              #
# =========================================================================== #
def _indicators_image_path() -> Optional[Path]:
    """Locate indicators.* next to the app or in the parent Test folder."""
    names = ("indicators", "Indicators")
    exts = (".png", ".jpg", ".jpeg", ".webp", ".svg")
    search_dirs = [_THIS_DIR, _THIS_DIR.parent, _THIS_DIR.parent.parent, Path.cwd()]
    seen: set[Path] = set()
...
def _run_streamlit_landing(st) -> None:
    st.markdown("""
        <style>
            .title-row {
                display: flex; align-items: baseline; flex-wrap: wrap;
                gap: 0.75rem 1.25rem; margin-bottom: 0.15rem;
            }
            .main-header {
            ...
            .intro-box p { margin: 0 0 0.65rem 0; }
            .intro-box p:last-child { margin-bottom: 0; }
            .intro-box ul { margin: 0.75rem 0 0 0; padding-left: 1.25rem; }
            .intro-box li { margin-bottom: 0.45rem; }
    ...
    st.markdown(
        '<div class="title-row">'
        '<div class="main-header">RECOPS</div>'
        '<span class="beta-badge">Beta version under development</span>'
        '</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="tagline">Resilience &amp; Cost benefits of Open-Source Software '
        'in the Power Sector</div>',
        unsafe_allow_html=True,
    )

    intro_html = """
        <div class="intro-box">
            <p><strong>RECOPS</strong> is an automated, evidence-based evaluation framework
            for open-source software in the power sector.</p>
            <p>It links public repository signals to engineering quality, project sustainability,
            governance, adoption, deployability, and energy-domain fitness.</p>
            <p>This methodology is applied to representative power-sector Open-Source Software
            projects and case-study repositories.</p>
            <p><strong>Scoring methodology</strong></p>
            <ul>
                <li><strong>Six families</strong> &mdash; Engineering Quality (EQ),
                Adoption &amp; Ecosystem (AE), Governance &amp; Credibility (GC),
                Deployability &amp; Interoperability (DI), Project Health (PH), and
                Energy-Domain Fitness (EDF).</li>
                <li><strong>27 sub-indicators</strong> &mdash; Each family combines headline
                indicators computed from repository evidence.</li>
                <li><strong>Automated extraction</strong> &mdash; Signals are read from the
                cloned repository, git history, and GitHub metadata.</li>
                <li><strong>0&ndash;100 scale</strong> &mdash; Higher scores reflect stronger
                observable evidence at evaluation time.</li>
                <li><strong>Geometric aggregation</strong> &mdash; A weak area is not masked
                by strong scores elsewhere within a family or composite.</li>
                <li><strong>Explicit missingness</strong> &mdash; Unavailable signals are
                excluded rather than treated as zero.</li>
            </ul>
        </div>
    """
    ...
    with col_image:
        indicators_img = _indicators_image_path()
        if indicators_img:
            st.image(str(indicators_img), width=400)
        else:
            st.caption(
                "Place `indicators.png` (or `.jpg`) in the same folder as `app.py` "
                "to show the indicator diagram."
            )

    _, col_btn, _ = st.columns([1, 2, 1])
    with col_btn:
        if st.button("Test Open-Source Software", type="primary", use_container_width=True):
            st.session_state.app_page = "scoring"
            st.rerun()


def _run_streamlit_scoring(st, pd) -> None:
    st.markdown("""
        <style>
            .main-header {
                background: -webkit-linear-gradient(45deg, #1e3a8a, #3b82f6);
    ...
    # ---- Up-front module health check ----
    mod_status = verify_modules()
    missing = {f: e for f, e in mod_status.items() if e}
    if missing:
        st.error("The six RECOPS scoring modules could not be imported, so scoring "
                 "will fail. Put all of recops_repo_scores_EQ/AE/GC/DI/PH/EDF.py in the "
                 "SAME folder as this app, then rerun.")
        with st.expander("Details"):
            st.write("App folder:", str(_THIS_DIR))
            st.write("Working dir:", str(Path.cwd()))
            for f, e in missing.items():
                st.write(f"- {f}: {e}")

    # ---- Config sidebar ----
    # Secrets/env credentials, if configured, are used silently and never
    # shown or prefilled in the UI. If not configured, the field is left
    # blank so a visitor can optionally paste a key for their own session.
    ...
    with st.sidebar:
        ...
        if secret_groq:
            ...
            groq_key = st.text_input(
                ...
                help="Set GROQ_API_KEY as an env var or in st.secrets. Needed for "
                     "the per-repo summary and the chat panel.")
    ...
    if st.button("Run RECOPS Scoring", type="primary", use_container_width=True):
        ...
        urls = deduped
        if not urls:
            st.warning("Please provide at least one repository URL.")
            st.stop()
        ...
        # Persist everything so the page survives chat reruns.
        ss.results = {"df": df, "flat_rows": flat_rows, "full_blobs": full_blobs,
                      "composites": composites, "ok": ok, "fail": fail, "total": len(urls)}
        ss.chat = []  # fresh chat for the new result set

    # ---- Render results + LLM panel (persist across reruns) ----
    if ss.results:
        r = ss.results
        df = r["df"]
        st.success("Scoring complete.")
        c1, c2, c3 = st.columns(3)
        c1.metric("Processed", r["total"])
        c2.metric("Succeeded", r["ok"])
        ...
        # Per-repo LLM summaries (visible, not hidden in a column)
        ...
        if not ss.groq_key:
            ...
            if quick[1].button("Strengths and weaknesses each", use_container_width=True):
                preset = (
                    "For each repository, and for every RECOPS family (EQ, AE, GC, DI, PH, EDF), "
                    "state one key strength and one key weakness. Use the family composite scores "
                    "and sub-indicator scores from the scorecard, cite the actual numbers, and "
                    "briefly explain what each strength and weakness implies."
                )
...
def _run_streamlit() -> None:
    ...
    st.set_page_config(page_title="RECOPS", page_icon="⚡", layout="wide")

    ss = st.session_state
    ss.setdefault("app_page", "landing")
    ss.setdefault("results", None)        # {"df": ..., "flat_rows": ..., "full_blobs": ...}
    ss.setdefault("chat", [])             # list of {"role","content"}
    ss.setdefault("groq_key", resolve_groq_key())
    ss.setdefault("model", GROQ_MODELS[0])

    if ss.app_page == "landing":
        _run_streamlit_landing(st)
    else:
        _run_streamlit_scoring(st, pd)
