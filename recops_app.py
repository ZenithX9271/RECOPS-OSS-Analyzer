...
# === Prompt & LLM Chain Setup (UPDATED TO LCEL) ===
...
llm = HuggingFaceHub(
    ...
    huggingfacehub_api_token='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', 
    task="text2text-generation"
)
# Modern LangChain syntax replacing LLMChain
chain = prompt | llm 
...
# === Run Prompt Chain (UPDATED TO INVOKE) ===
def run_chain(context_block, selected_question):
    return chain.invoke({"context": context_block, "q": selected_question})
    ...
    if st.button("Run this insight"):
        if "analysis_result" in st.session_state and st.session_state.analysis_result:
            try:
                ...
                # Fetch API key to use
                groq_key = GROQ_API_KEY if GROQ_API_KEY else os.getenv("GROQ_API_KEY")
                
                llm = ChatGroq(api_key=groq_key, model="llama-3.1-8b-instant")
                
                # Updated prompt and chain for LCEL
...
"""
                ...
                # Replace LLMChain with the | operator
                insight_chain = insight_prompt | llm
                
                # Run using invoke
                result = insight_chain.invoke({"context": context_block, "q": selected_question})
                
                st.success("Insight:")
                
                # Handle output safely depending on if it's an AIMessage object or a string
                if hasattr(result, 'content'):
                    st.write(result.content.strip())
                else:
                    st.write(str(result).strip())
